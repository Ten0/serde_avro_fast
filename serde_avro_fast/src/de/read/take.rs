...
impl Read for SliceReadTake<'_> {
	...
	fn skip_bytes(&mut self, n_bytes: u64) -> Result<(), DeError> {
		self.inner_slice_read.skip_bytes(n_bytes)
	}
}
#[cfg(feature = "std")]
...
impl std::io::Read for SliceReadTake<'_> {
	fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
		std::io::Read::read(&mut self.inner_slice_read.slice, buf)
	}
	fn read_vectored(&mut self, bufs: &mut [std::io::IoSliceMut<'_>]) -> std::io::Result<usize> {
		std::io::Read::read_vectored(&mut self.inner_slice_read.slice, bufs)
	}
}
#[cfg(feature = "std")]
impl std::io::BufRead for SliceReadTake<'_> {
	fn fill_buf(&mut self) -> std::io::Result<&[u8]> {
		std::io::BufRead::fill_buf(&mut self.inner_slice_read.slice)
	}

	fn consume(&mut self, amt: usize) {
		std::io::BufRead::consume(&mut self.inner_slice_read.slice, amt)
	}
}
