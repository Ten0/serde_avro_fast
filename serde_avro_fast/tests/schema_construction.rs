...
#[test]
fn manually_constructed_schema_mut_allows_cycles() {
	let schema_mut = SchemaMut::from_nodes(vec![
	...
	assert_eq!(
		match &schema_mut.root().type_ {
			RegularType::Record(record) => record.fields[0].type_,
			other => panic!("expected root record, got {other:?}"),
		},
		SchemaKey::from_idx(1)
	);
	assert_eq!(
		match &schema_mut[SchemaKey::from_idx(1)].type_ {
			RegularType::Record(record) => record.fields[0].type_,
			other => panic!("expected dependent record, got {other:?}"),
		},
		SchemaKey::root()
	);
}
...
#[test]
fn manually_constructed_schema_mut_allows_unreferenced_nodes() {
	let schema_mut = SchemaMut::from_nodes(vec![
		SchemaNode::new(RegularType::Record(Record::new(
			Name::from_fully_qualified_name("Main"),
			vec![RecordField::new("f", SchemaKey::from_idx(1))],
		))),
		SchemaNode::new(RegularType::Int),
		SchemaNode::new(RegularType::Record(Record::new(
			Name::from_fully_qualified_name("Unused"),
			vec![RecordField::new("w", SchemaKey::from_idx(1))],
		))),
	]);

	assert_eq!(schema_mut.nodes().len(), 3);
}
