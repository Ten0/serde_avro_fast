...
//! (de)serialization do tons of unnecessary allocations, `HashMap` lookups, etc... for every record they encounter.
//!
...
/// This is zero-alloc.
///
/// Your structure may contain `&'a str`s that will end up pointing directly into this slice for ideal performance.
...
///
/// If deserializing from a slice, a `Vec`, ... prefer using `from_datum_slice`, as it will be more performant
/// and enable you to borrow `&str`s from the original slice.
