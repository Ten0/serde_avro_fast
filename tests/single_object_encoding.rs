...
lazy_static! {
	...
	static ref APACHE_SCHEMA: apache_avro::Schema = apache_avro::Schema::parse_str(SCHEMA_STR).unwrap();
}
...
impl From<TestSingleObjectReader> for Value {
	fn from(obj: TestSingleObjectReader) -> Value {
		Value::Record(vec![
			("a".into(), obj.a.into()),
			("b".into(), obj.b.into()),
			("c".into(), Value::Array(obj.c.into_iter().map(|s| s.into()).collect())),
		])
	}
}

fn encode(value: impl Into<Value>, schema: &apache_avro::Schema, out: &mut Vec<u8>) -> apache_avro::AvroResult<()> {
...
fn test_avro_3507_single_object_reader() {
	...
	to_read.extend_from_slice(&APACHE_SCHEMA.fingerprint::<apache_avro::rabin::Rabin>().bytes[..]);
	encode(expected_value.clone(), &APACHE_SCHEMA, &mut to_read).expect("Encode should succeed");
	let val: TestSingleObjectReader = from_single_object_slice(to_read.as_slice(), &SCHEMA).unwrap();
...
fn avro_3642_test_single_object_reader_incomplete_reads() {
	...
	to_read_2.extend_from_slice(&APACHE_SCHEMA.fingerprint::<apache_avro::rabin::Rabin>().bytes[..]);
